...
uses
  ...
  System.Net.HttpClientComponent,json,Vcl.Graphics , Soap.EncdDecd,Vcl.Imaging.pngimage, UEmoticons, System.IOUtils;
...
type
  ...
  private
    FSecretKey: string;
    FSession: string;
    FCaminhoQrCode: string; //TPath.GetTempPath
    FURLBase: string;
    FPorta: string;
  ...
  public
       ...
       function SendButtons(Telefone,CorpodaMensagem,TelefoneLigar,TextoBotaoTelefone,LinkRedirecionamento,TextodoLink,TituloMensagem, RodapeDaMensagem: string): Boolean;
...
constructor TDTWpp.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);

  FCaminhoQrCode := TPath.GetTempPath;
end;

destructor TDTWpp.Destroy;
begin
...
function TDTWpp.SendButtons(Telefone, CorpodaMensagem, TelefoneLigar,
  TextoBotaoTelefone, LinkRedirecionamento, TextodoLink, TituloMensagem,
  RodapeDaMensagem: string): Boolean;
const
  REQUEST_BODY =
  ' { ' +
  ' "phone": "%s", ' + // telefone
...
begin
    try
      ...
      try
        ...
        end else if Response.StatusCode = 404 then
        begin
           FrmqrCode := TFrmqrCodeWpp.Create(nil);
           FrmqrCode.Token            := RetornoAuth.token;
           FrmqrCode.Session          := FSession;
           FrmqrCode.SecretKey        := FSecretKey;
           FrmqrCode.URLBase          := FURLBase;
           FrmqrCode.Porta            := FPorta;
           FrmqrCode.pnStatus.Caption := '';
           FrmqrCode.ShowModal;
        end else begin
           Result := False;
           raise Exception.Create(Response.StatusCode.ToString + ' ' + Response.StatusText + ' ' + Response.ContentAsString);
...
begin
    ...
    try
      ...
      REQUEST_BODY := '{ "phone": "'+telefone+'","base64": "'+'data:application/'+ ExtractFileExt(FileName).Replace('.','') +';base64,' +sBase64+'","caption": "'+mensagem+'","filename": "'+  FileName +'", "isGroup": false }';
...
begin
    try
      ...
      Mensagem := Mensagem.Replace(#$D,'').Replace(#$A,'');
      ...
      try
        ...
        begin
           Result             := True;
           Retorno.Mensagem   := Response.ContentAsString;
        end else if ( Response.StatusCode = 400 ) or ( Response.StatusCode = 404 ) then
        begin
           FrmqrCode := TFrmqrCodeWpp.Create(nil);
           FrmqrCode.Token            := RetornoAuth.token;
           FrmqrCode.Session          := FSession;
